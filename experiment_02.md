...
We kept $D_t = D_0 \forall t$ to ensure determinism
...
4.  But for $D_0$, the value at which D_0 starts and since $D_t = D_0 \forall t$, should be the same for the 3 memory modes in each run, and also for different runs (as per our code's random seed setting), But we are getting different values because:
